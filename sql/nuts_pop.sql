	...
	POP_NUTS AS (
		SELECT
			*,
			...
			"2023",
			regexp_extract("2023", '\d+', 0) AS POP_2023,
			regexp_extract("2022", '\d+', 0) AS POP_2022,
			regexp_extract("2021", '\d+', 0) AS POP_2021,
			regexp_extract("2020", '\d+', 0) AS POP_2020,
			regexp_extract("2019", '\d+', 0) AS POP_2019
		FROM
			POP_RAW
	),
	POP_NUTS_CLEAN AS (
